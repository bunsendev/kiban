...
def install_daily_routes(app: FastAPI, authorize, daily) -> None:
    ...
    def create_closed_day(
        request: ClosedDayCreate,
        principal: Annotated[Principal, Depends(approve)],
    ):
        try:
            value = make_closed_day(**audit_payload(request, principal, "approved_by"))
    ...
    def create_daily_build(
        request: DailyBuildCreate,
        _principal: Annotated[Principal, Depends(analyze)],
    ):
        try:
