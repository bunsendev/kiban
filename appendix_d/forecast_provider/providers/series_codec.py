"""日次float64 Seriesを欠損と確定0を区別してJSON化する。"""
...
def decode_series(value: dict) -> pd.Series:
    ...
    try:
        ...
        raise ArtifactError("seriesの日付範囲が不正です") from exc


def encode_series_map(value: dict) -> dict:
    if not isinstance(value, dict):
        raise ArtifactError("series集合が不正です")
