"""StatsForecast AutoETSを固定学習契約へ接続するProvider。"""
...
MODEL_ID = "auto_ets_weekly"
MODEL_METADATA = ModelMetadata(
    model_id=MODEL_ID,
    display_name="StatsForecast AutoETS（日次・週周期）",
    min_history_days=28,
    supported_horizons=(1, 400),
    supports_intervals=False,
    primary=True,
)
SUPPORTED_MODELS = {MODEL_ID: MODEL_METADATA}


class StatsForecastETSProvider(ForecastProvider):
    """TRAINで選択・推定したETSを、各起点の履歴へ固定適用する。"""
    ...
    def metadata(self) -> ProviderMetadata:
        return ProviderMetadata(
            ...
            runtime_dependencies=tuple(
                ...
                for name in ("statsforecast", "coreforecast", "numpy", "pandas")
            ),
        )
    ...
    def validate(self, dataset: ForecastDataset, config: ProviderConfig) -> ValidationResult:
        ...
        if model_meta is None:
            issues.append(_issue("STATSFORECAST_UNKNOWN_MODEL", "未知のモデル指定です"))
        elif not model_meta.accepts_horizon(dataset.max_horizon):
            issues.append(_issue("STATSFORECAST_HORIZON_OUT_OF_RANGE", "horizonが対応範囲外です"))
        if dict(config.params) != MODEL_PARAMS:
            issues.append(
                _issue(
                    "STATSFORECAST_PARAMS_MISMATCH",
        ...
        if config.preprocessing_version != PREPROCESSING_VERSION:
            issues.append(
                _issue(
                    "STATSFORECAST_PREPROCESSING_MISMATCH",
        ...
        if config.interval_levels:
            issues.append(
                _issue(
                    "STATSFORECAST_INTERVALS_UNSUPPORTED",
        ...
        if dataset.known_future_columns:
            issues.append(
                ValidationIssue(
                    "STATSFORECAST_EXOGENOUS_IGNORED",
    ...
    def fit_parameters(
        ...
        for uid in dataset.unique_ids:
            ...
            if group is None:
                exclusion_reasons[uid] = "NO_TRAIN_ROWS"
                continue
            series, actual_count, imputed_count = prepare_daily_series(
                group, dataset.train_start, dataset.train_end
            )
            if actual_count < MODEL_METADATA.min_history_days:
                exclusion_reasons[uid] = "INSUFFICIENT_HISTORY"
                continue
            try:
                fitted = fit_auto_ets(series, dict(config.params))
                signature = model_signature(fitted)
            except Exception as exc:
                exclusion_reasons[uid] = f"MODEL_FIT_FAILED:{type(exc).__name__}"
                context.logger.warning("AutoETS fit失敗: uid=%s error=%s", uid, type(exc).__name__)
                continue
            models[uid] = fitted
        ...
        if not models:
            failed = [
                uid
                for uid, reason in exclusion_reasons.items()
                if reason.startswith("MODEL_FIT_FAILED:")
            ]
            if failed:
                raise NonRetryableProviderError(
                    f"AutoETSの学習が全対象で失敗しました: {sorted(failed)}"
                )
            raise InsufficientHistoryError("AutoETSを学習できる系列がありません")
    ...
    def refresh_context(
        ...
        signatures = self._verified_signatures(model_ref)
        return ContextRef(
    ...
    def predict(
        ...
        if context_ref.model_id != model_ref.model_id:
            raise ContractViolationError("context_refが別のモデルに紐づいています")
        ...
        for uid, targets in future_df[future_df.horizon.isin(horizons)].groupby(
            "unique_id", sort=True
        ):
            ...
            forecast = forward_auto_ets(fitted, series, maximum)
    ...
    def _validate_model_ref(model_ref: ModelRef) -> None:
        if (
            ...
            raise ContractViolationError("StatsForecast ModelRefの識別条件が不正です")

    @staticmethod
    def _verified_signatures(model_ref: ModelRef) -> dict[str, str]:
        actual = {uid: model_signature(model) for uid, model in model_ref.state["models"].items()}
...
def build() -> StatsForecastETSProvider:
    return StatsForecastETSProvider()
