...
> 更新（2026-09-26）: 本書の調査後にPhase 3S-1〜3S-11を実装した。現在は`FACTORY` / `WAREHOUSE`、`EXPIRY_BUCKET`、CASE、snapshot、隔離・数量照合、API / Worker、確認済み商品mapping台帳、確認済みlocation master、Inventory Input Mapping、ファイル名日付からsnapshot日時を生成する版付きpolicyまで実装済みである。以下の「未対応」は調査時点の記録として残し、最新の実装結果は`planning/PHASE3S1_RESULT.md`〜`planning/PHASE3S11_SNAPSHOT_TIME_POLICY_RESULT.md`を正とする。実データ147商品の業務確認、2倉庫の正式承認、snapshot締め時刻、FACTORY在庫、生産予定、正式route policyが未完了のため、Phase 3Tの業務受入はまだ開始できない。
