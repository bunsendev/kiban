...
`http://127.0.0.1:58000/ui/easy`は、PC操作に不慣れな担当者向けの入口である。現在はCSV・ZIPのアップロード、または管理対象フォルダー内のCSV選択から、互換性のある列mappingを自動選択して検証jobを登録するところまでを1画面で行う。token、mapping ID、job IDなどの管理情報は通常操作から隠し、元ファイルを変更しないことを明示する。詳細は[担当者向けかんたん予測画面](docs/担当者向けかんたん予測画面.md)と[担当者向け操作マニュアル](docs/担当者向け操作マニュアル.md)を参照する。
