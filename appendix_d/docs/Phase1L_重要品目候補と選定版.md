...
リポジトリには実データが配置されていないため、匿名3品目とPostgreSQL実DBで算出・確定手順を確認した。後続の[Phase 1V専用画面](Phase1V_重要品目選定画面.md)で候補job登録、指標確認、選定版確定、履歴確認を行える。実データ全3年の候補算出、20〜50品目の業務確定、選定版からの日次build再発行は未実施である。
