...
## 24. v2.9 Phase 2D 追加実装

`kiban-db drill`は、PostgreSQL custom archiveを内部生成の一時DBにだけ復元する。元DBを復元先とする入力を持たず、一時DB名を固定patternで検査し、成否にかかわらず強制削除する。

backup前後の元DB指紋を照合し、archiveのsize・checksum・一覧、復元、schema・sequence・全table行の指紋一致、一時DB削除を6判定にする。行出力はstreaming SHA-256にし、行値をmemoryへ蓄積しない。内容アドレス方式証跡にDSN、DB/user名、path、一時DB名、relation名、行値、例外messageを含めない。詳細は[Phase 2D PostgreSQL隔離リカバリ訓練](Phase2D_PostgreSQL隔離リカバリ訓練.md)を参照する。

開発・検証環境での合格は本番RTO・RPO、災害復旧、実データ復旧の達成を表さない。

## 25. v2.9 Phase 2E 追加実装

全7管理画面は、従来の手入力Bearer tokenに加え、OIDC public clientのAuthorization Code Flow with PKCE S256でaccess tokenを取得できる。state、verifier、戻り先、作成時刻だけをsessionStorageに5分保持し、callback開始時に削除する。codeとerrorのqueryも直ちにURLから消去する。

APIは固定HTTPS token endpointと同一originから再構成したredirect URIだけを受付け、code・verifierをform POSTする。refresh token・ID tokenはブラウザーへ返さない。access tokenは既存API clientのmodule memoryだけに保持し、URL、cookie、localStorage、sessionStorageへ保存しない。詳細は[OIDC PKCEログイン](Phase2E_OIDC_PKCEログイン.md)を参照する。

## 26. v2.9 Phase 2F 追加実装

`kiban-oidc-preflight`はissuerから標準Discovery URLを導出し、設定済みJWKSとともにsystem trust store・hostname検証付きHTTPSで取得する。redirectは許可せず、既定timeout 5秒、応答128 KiBを上限とする。取得失敗時も接続先や例外本文を証跡へ含めず`BLOCKED`として保存する。

issuer、authorization endpoint、token endpoint、JWKS URIの設定一致、全endpointのHTTPS、response type `code`、Authorization Code grant、PKCE `S256`、許可非対称署名方式、JWKSの1〜100鍵、互換署名鍵、有効で重複のない`kid`を14項目で検査する。全件合格だけを`READY_FOR_IDP_LOGIN`とする。証跡はURL、host名、公開鍵値を保存せず、設定URLのSHA-256、制限、方式、判定、件数、観測時間を内容アドレス方式JSONへ追記する。詳細は[外部IdP接続プリフライト](Phase2F_外部IdP接続プリフライト.md)を参照する。

この判定は公開metadataと公開鍵の取得時点の技術互換性である。client・redirect URI・利用者・audience・roleの登録、管理者同意、実token発行、logout・失効連携は環境固有IdPによる本番受入で確認する。

## 27. v2.9 Phase 2G 追加実装

`kiban-mapping-dry-run`は、実業務原本を取込台帳へ登録する前に、管理対象入力root内のCSVと明示的な列マッピングJSONをread-onlyで検査する。絶対path、親directory移動、symlink、空ファイル、上限超過を拒否し、Phase 1Gと同じ厳密な文字コード判定とPhase 1Hと同じヘッダー・行正規化規則を共用する。

既定の先頭1,000行について採用・隔離件数、固定code別の隔離理由、解釈可能数量の採用行・隔離行への照合を判定する。構造・契約違反は`BLOCKED`、隔離行ありは`REVIEW_REQUIRED`、全判定合格は`READY_FOR_NORMALIZATION`とする。ドライランはPostgreSQL、原本archive、取込・正規化台帳を更新しない。

内容アドレス方式のJSON証跡には入力とマッピングのSHA-256、制限、文字コード、判定、件数だけを保存する。原本path、ファイル名、ヘッダー名、JAN、商品名、center、日付、数量、行値、例外本文を含めない。Compose serviceはnetworkを無効化し、入力rootとマッピングをread-only mountする。詳細は[クレンジング・列マッピングドライラン](Phase2G_クレンジング列マッピングドライラン.md)を参照する。

`READY_FOR_NORMALIZATION`は検査したサンプルの技術判定であり、サンプル外の全行品質、実データ受入、業務承認、予測精度を保証しない。実取込ではPhase 1G〜1Hによる原本保存、版採用、全行正規化、隔離、全件数量照合を改めて実施する。

## 28. v2.9 Phase 2H 追加実装

APIは`KIBAN_MAPPING_DRY_RUN_DIR`をread-onlyで参照し、内容アドレス方式のPhase 2G証跡だけを一覧・詳細として公開する。証跡ごとに通常ファイル、256 KiB上限、64桁小文字SHA-256のファイル名、内容checksum、厳密UTF-8、重複JSON keyなし、固定top-level schema、format version、固定判定・隔離理由code、条件から再計算したdry run ID、判定と検査結果の整合性を再検証する。改変・未知項目・不整合がある証跡は一覧から除外して件数だけを返し、個別参照は409にする。

認証済みREAD利用者は`GET /api/mapping-dry-runs`と`GET /api/mapping-dry-runs/{report_sha256}`を使用する。応答はreport・dry run・mapping・原本のSHA-256、実行時刻、上限、検査IDと状態、採用・隔離件数、固定隔離理由code、固定制約へ限定する。証跡内の実測値・期待値、原本path、ファイル名、ヘッダー名、JAN、商品名、center、日付、数量、行値、例外本文を返さない。

`/ui/intake`はドライラン一覧、検証済み・除外件数、総合判定、件数、検査状態、隔離理由、制約を表示する。ブラウザーは証跡ファイルを直接読まず、認証付きAPIの検証済み応答だけを扱う。開発・本番ComposeはAPIへ証跡rootをread-only mountし、Phase 2G実行serviceだけに同じrootへの書込みを許可する。詳細は[マッピングドライラン証跡レビュー](Phase2H_マッピングドライラン証跡レビュー.md)を参照する。

表示される判定はサンプルの技術結果であり、サンプル外の全行品質、実データ受入、業務承認、予測精度を保証しない。実取込ではPhase 1G〜1Hの保存、版採用、全行正規化、隔離、全件数量照合を改めて実施する。

## 29. v2.9 Phase 2I 追加実装

ローカルデータ検証要求は`mapping_dry_run_jobs`へ永続化し、HTTP request内でCSVを解析しない。要求は入力rootからの相対path、登録済みmapping ID、1〜10,000行のsample上限、認証subjectを含む。状態は`QUEUED`、`RUNNING`、`SUCCEEDED`、`FAILED`とし、成功時はdry run ID、判定、report SHA-256を保持する。

専用WorkerはPostgreSQLの`FOR UPDATE SKIP LOCKED`で1件をclaimし、Phase 2Gと同じpath、文字コード、ヘッダー、必須列、sampleクレンジング、数量照合を実行する。入力不備は`BLOCKED`証跡として正常完了させ、予期しない基盤障害だけを固定error codeの`FAILED`とする。job・証跡へ原値と例外本文を保存しない。

`/ui/intake`は登録済みmappingを選択する検証job form、job状態・登録者・時刻・判定・証跡識別子、checksum検証済みレポートを表示する。開発・本番ComposeのWorkerは入力rootをread-only、証跡rootだけをwrite mountする。APIは証跡rootをread-onlyで参照する。詳細は[ローカルデータ検証UI](Phase2I_ローカルデータ検証UI.md)を参照する。
# Phase 3P 追補: レビュー対応タスク

精度変化レビューの次の対応は、`model_review_actions` の不変なタスクと `model_review_action_events` の追記イベントで管理する。イベントはタスク単位の連番revision、状態、担当者、期限、内容、完了根拠、認証subject、記録時刻を持つ。完了には根拠を必須とし、完了・中止後の変更を拒否する。更新APIは期待revisionを要求し、同時更新を409で拒否する。詳細は[Phase3P_レビュー対応タスク.md](Phase3P_レビュー対応タスク.md)を参照する。

# Phase 3Q 追補: レビュー追加テスト自動連携

`RETEST`対応は`model_review_retests`で元キャンペーン、対象snapshot、新キャンペーンへ不変に結び付ける。開始要求は認証subject、request key、期待revisionを検証し、元キャンペーンのモデル集合と評価条件を再利用する。比較キャンペーンWorkerは未同期結果をpollし、成功時の比較IDまたは安全化した失敗理由を対応イベントと同じtransactionで追記する。詳細は[Phase3Q_レビュー追加テスト自動連携.md](Phase3Q_レビュー追加テスト自動連携.md)を参照する。

# Phase 3R 追補: 追加テスト差分と再レビュー

成功済み追加テストは、元キャンペーンと追加キャンペーンの保存済み公式共通指標を`provider_id`・`model_id`単位で照合する。WAPE、MAE、RMSE、絶対Bias、成功率、順位の元値・追加値・差分を返し、欠損は欠損のまま保持する。両側が正式対象でWAPEが存在する場合だけ改善・悪化・変化なしを判定し、それ以外は比較不能とする。

分析画面は差分表と判定根拠を表示し、レビュー対象モデルが現在の比較可能系列に存在する場合だけ、新しい判断版フォームへ差分を引き継ぐ。引継ぎ時点では保存せず、判断区分・根拠・対応を利用者が確認して版付きレビューとして登録する。採用、昇格、rollbackを自動変更しない。詳細は[Phase3R_追加テスト差分と再レビュー.md](Phase3R_追加テスト差分と再レビュー.md)を参照する。
